class QChat {
         ...
         allGroups.forEach(function(list) {
          ...
          $(`${elem} .chat-list-person .content`).click(function() {
			...
			window.onresize = function() {
				height = $(`${elem} .chat-box`).height() - 117;
				$(`${elem} .chat-content-list`).slimScroll({height: height});
			}
  ...
  static openChat({toId, myName, toName, settings, message})  {
    db.collection('group').where('members', '==', [settings.myId, toId]).get().then(function(data) {
      if(data.docs.length) {
        let groupId = data.docs[0].id;
        let elem = settings.elem;
        let height = $(`${elem} .chat-box`).height() - 117
        
        $(`${elem} .chat-bubble`).click();
      
        $(`${elem} .chat-list-person`).slimScroll({height: '0'});
        $(`${elem} .chat-content-list`).slimScroll({height: height});
        $(`${elem} .chat-content-list`).css({height: height});
        $(`${elem} .chat-content`).show();
        $(`${elem} .chat-back`).show();
        $(`${elem} #chat-input`).attr('groupId', groupId)
        $(`${elem} #btn-send`).attr('groupId', groupId)
        $(`${elem} .chat-title`).text(toName)
        
        QChat.getChat(settings.myId, groupId, elem, settings.color);
        QChat.markAsRead(settings.myId, groupId);
        
      } else {
        db.collection('group').where('members', '==', [toId, settings.myId]).get().then(function(data) {
          if(data.docs.length) {
            let groupId = data.docs[0].id;
            let elem = settings.elem;
            let height = $(`${elem} .chat-box`).height() - 117
